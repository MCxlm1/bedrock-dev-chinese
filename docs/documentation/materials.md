...
:::warning
材质不适合初学者。请准备好应对潜在的崩溃、内容日志错误和长时间加载。
:::

材质对于使实体更加独特非常有用。您可以为您的附加组件创建新的材质，或使用现有的原版材质。

您可以[这里](file:///c/Users/MCxlm/Downloads/bedrock-wiki-wiki/bedrock-wiki-wiki/docs/visuals/materials)了解更多关于创建材质的信息。
...
使纹理不受暗淡照明的影响，看起来像是发光的。如果有使用alpha通道，发光度与每个像素的透明度成正比。

### 设置透明度

无论其他属性如何，始终以预设的透明度完全渲染。

### 纹理混合

当存在多个纹理时，可能会使用某种滤镜根据纹理更改实体的外观。

## 材质详情

以下是每个材质的列表，以及已知的一般属性。名称是对每个材质功能的模糊提示，有些可能表现得相当不可预测，或有未记录的用途，因此这只是每个材质的确定内容：

:::warning
以下部分目前**仅**针对单个纹理进行了测试。请谨慎看待这些信息。强烈建议您自己试验这些材质。
:::
...
:::warning
请注意，这些也仅使用RenderDragon平台进行了测试。非RenderDragon视觉效果可能有所不同。
:::